...
export async function buildTransferTokensTransaction(
  ...
  const multisigInfo = await token.getMultisig(connection(), sourceInfo.owner);
  if (!multisigInfo) {
...
<template>
  <Layout :header="true">
    ...
    <div v-else class="grid place-items-center py-20">
      ...
      <div class="max-w-5xl">
        <p class="font-display text-3xl tracking-tight text-white">
          Build A Transfer Tokens Transaction
        </p>

        <div>
          <p class="mt-3 mb-2 text-lg tracking-tight text-slate-400">
            This page will help you build a transaction to transfer tokens from
            a multi-sig token account to a destination, you'll need to get
            signatures from the multi-sig account holders.
          </p>
