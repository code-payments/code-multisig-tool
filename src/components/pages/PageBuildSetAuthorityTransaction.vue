...
export async function buildSetAuthorityTransaction(
  mintConfig: {
...
export default {
  ...
  methods: {
    ...
    async onConfirm() {
      ...
      const newAuthority = new web3.PublicKey(this.newAuthority);
      const signed = await buildSetAuthorityTransaction({
        address: mintAddress,
        newAuthority,
      },
      signers,
    ...
    isValid() {
      ...
      if (!this.newAuthority) {
        console.log("no new authority");
        return false;
      }
...
<template>
  <Layout :header="true">
    ...
    <div v-else class="grid place-items-center py-20">
      ...
      <div class="max-w-5xl">
        <p class="font-display text-3xl tracking-tight text-white">
          Build A Set Authority Transaction
        </p>

        <div>
          <p class="mt-3 mb-2 text-lg tracking-tight text-slate-400">
            This page will help you transfer ownership of a multi-sig mint to a
            new address, you'll need to get signatures from the multi-sig
            account holders.
          </p>
          ...
          <h3 class="text-2xl font-medium leading-6 text-white mt-10">New Authority (base58)</h3>
          <p class="mt-3 text-sm text-slate-400">
            Who will be the new authority for the mint?
          </p>  
          <input v-model="newAuthority" type="text"
            class="mt-3 p-2 text-md tracking-tight text-slate-400 rounded-xl border border-slate-200 dark:border-slate-800 w-full bg-transparent outline-none" />
